...
const PollingReview = ({ polls, network }: PollingReviewProps) => {
  ...
  const votesToTweet = (): string => {
    ...
    if (modalPollId) {
      ...
      url = `https://vote.dijets.io/polling/${poll.slug}`;
      text = `I just voted ${
        option ? option + ' ' : ''
      }on a Dijets governance poll! Learn more about the poll on the Governance Portal:`;
    } else {
      // all votes
      url = 'https://vote.dijets.io';
      text = `I just voted on ${
        previousVotesLength > 1 ? previousVotesLength : 'a'
      } Dijets governance poll${
        previousVotesLength > 1 ? 's' : ''
      }! Find my votes and all Dijets Governance proposals on the Governance Portal:`;
    }
  ...
  const votesToMarkdown = (): string => {
    ...
    polls.map(poll => {
      ...
      markdown += `[${poll.title}](https://vote.dijets.io/polling/${poll.slug}) ([thread](${poll.discussionLink}))  \n`;
...
export const getStaticProps: GetStaticProps = async () => {
  const { polls } = await fetchPollingPageData(SupportedNetworks.DIJETS);
