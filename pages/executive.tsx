...
import { DijetsSdk } from '@dethcrypto/eth-sdk-client';
...
export const ExecutiveOverview = ({ proposals }: { proposals?: Proposal[] }): JSX.Element => {
  ...
  const { chief } = useContracts() as DijetsSdk;
  ...
  const { data: lockedMkrOldChief } = useSWR(
    ...
    () => chief.deposits(lockedMkrKeyOldChief as string)
  );
    ...
    <PrimaryLayout sx={{ maxWidth: [null, null, null, 'page', 'dashboard'] }}>
      ...
      {votedProposals &&
        ...
        lockedMkr.gt(0) && (
          <>
            <ProgressBar step={2} />
            <MigrationBadge>Your HAL has been deposited into the Governance Contract. You are now ready to vote.</MigrationBadge>
          </>
        )}
      <Stack>
        {account && (
        ...
        <SidebarLayout>
          ...
          <Stack gap={3}>
            <Box sx={{ mb: 3 }}>
            <Card variant="compact" sx={{background: 'linear-gradient(158.87deg, rgb(226, 247, 249) 0%, rgb(211, 243, 245) 100%), rgb(255, 255, 255)', padding: '18px 24px 20px', fontSize: '14px', color: '#8386ab'}}>
                <Text as="p" sx={{ mb: 3, color: 'textSecondary' }}>
                  It is also possible to vote on a custom mandate address.
                </Text>
                <Box>
                  <InternalLink href={'/custom-spell'} title="View custom spell voting page">
                    <Text color="#1e1e1e" sx={{ fontWeight: 'bold'}}>View Custom Mandate Voting Page</Text>
                  </InternalLink>
                </Box>
              </Card>
            </Box>
            ...
            <ErrorBoundary componentName="DJTX Liquidity">
...
export default function ExecutiveOverviewPage({
  ...
  if (error && error.status === 404) {
    return (
      <PrimaryLayout sx={{ maxWidth: 'dashboard' }}>
        <ErrorPage statusCode={404} title="Mandate not found" />
      </PrimaryLayout>
    );
  } else if (error) {
    return (
  ...
  return (
    <ErrorBoundary componentName="Mandates Page">
...
export const getStaticProps: GetStaticProps = async () => {
  const { proposals } = await fetchExecutivePageData(SupportedNetworks.DIJETS);
