...
const Delegates = ({ delegates, stats, tags }: DelegatesPageData) => {
  ...
  return (
    <PrimaryLayout sx={{ maxWidth: [null, null, null, 'page', 'dashboard'] }}>
      <HeadComponent
        title="Delegates"
        description="Vote delegation allows for HAL holders to delegate their voting power to other members, which increases the effectiveness and efficiency of the governance process."
        image={'https://tokens.dijets.io/delegates.png'}
      />
      <Stack>
        ...
        <SidebarLayout>
            ...
            <Stack gap={3}>
              {sortedDelegates && sortedDelegates.length === 0 && (
                <Flex sx={{ flexDirection: 'column', alignItems: 'center', pt: [5, 5, 5, 6] }}>
                  ...
                  <Button
                    variant={'textual'}
                    sx={{ color: '#868685', textDecoration: 'underline', mt: 2, fontSize: '11px' }}
                    onClick={resetFilters}
                  >
                    Reset filters
                  </Button>
          ...
          <Stack gap={3}>
            <Box>
              ...
              <Card variant="compact" sx={{background: 'linear-gradient(171.29deg, rgb(253, 222, 240) -2.46%, rgb(255, 240, 249) -2.45%, rgb(255, 246, 241) 99.08%)', padding: '18px 24px 20px', fontSize: '14px', color: '#787a9b'}}>
                <Text as="p" sx={{ mb: 3, fontSize: '14px', color: '#7f7f8c' }}>
                ...
                <Box>
                  ...
                  >
                    <Text color="#181818" sx={{ fontWeight: 'bold' }}>View Account Page</Text>
                  </InternalLink>
                </Box>
              </Card>
            </Box>
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const { delegates, stats, tags } = await fetchDelegatesPageData(SupportedNetworks.DIJETS);

  return {
    revalidate: 60, // allow revalidation every 30 minutes
    props: {
