...
export const useEsmBurn = (): BurnResponse => {
  ...
  const burn = (burnAmount, callbacks) => {
    ...
    const txId = track(burnTxCreator, account, 'Burning HAL in Emergency Shutdown Module', {
      initialized: () => {
      ...
      mined: txId => {
        transactionsApi.getState().setMessage(txId, 'Burned HAL in Emergency Shutdown Module');
