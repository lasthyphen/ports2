...
const MeetDelegateCard = ({
  ...
  return (
    ...
    >
      ...
      >
        <InternalLink href={`/address/${delegate.voteDelegateAddress}`} title="View members profile details">
          <Button
            sx={{
              minWidth: '186px',
              borderRadius: 'round',
...
export const ParticipationBreakdown = ({
  ...
  return (
    <Flex sx={{ flexDirection: 'column' }}>
      ...
      <Flex
        sx={{ justifyContent: 'space-between', flexDirection: bpi === 0 ? 'column' : 'row', mt: 4, gap: 2 }}
      >
        <Flex sx={{ flexDirection: 'column' }}>
          <Text as="p" sx={{ fontWeight: 'bold', fontSize: '14px' }}>
      ...
      >
        ...
        >
          <Flex sx={{ maxWidth: 'page' }}>
            <Heading>DGC Members</Heading>
            <Flex sx={{ flex: 1, justifyContent: 'flex-end' }}>
              <InternalLink href="/delegates" title="View All DGC Members">
                <ViewMore label="View All" />
              </InternalLink>
            </Flex>
          </Flex>
          <Text variant="secondary" sx={{ width: ['100%', '100%', '50%'] }}>
            Vote delegation allows anyone with HAL to delegate their voting power to their preferred council member, which
