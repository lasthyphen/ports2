...
const AccountSelect = (): React.ReactElement => {
  ...
  const handleError = e => {
    ...
    if (e.toString().includes('NoSafeContext') || e.toString().includes('safe context')) {
      message = 'Please try connecting from within the Dijets Vault app.';
    } else {
  ...
  const walletOptions = Object.keys(SUPPORTED_WALLETS).map((connectionName: WalletName, index) => (
    ...
    >
      <Flex sx={{ alignItems: 'center' }}>
        <Icon name={SUPPORTED_WALLETS[connectionName].name} color="text" size="24px" />
          ...
          {changeWallet ? (
            ...
            <>
              <Flex sx={{ flexDirection: 'row', justifyContent: 'space-between', mb: 3, mt: 1 }}>
                <Text variant="microHeading">{address ? 'Account' : 'Connect your Wallet'}</Text>
              ...
              {address ? (
                <>
                  ...
                  <Button
                    ...
                    onClick={close}
                    sx={{ display: ['block', 'none'], width: '100%', mt: 4, mb: 2, py: 2 }}
                  >
                    Close
                  </Button>
                </>
