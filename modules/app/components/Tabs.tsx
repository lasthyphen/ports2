  ...
  return (
    ...
    >
      <Tabs index={activeTabIndex} onChange={index => setActiveTabIndex(index)}>
        <TabList sx={{ display: ['flex', 'block'], marginTop: '10px', bg: 'inherit', ...tabListStyles }}>
          {tabRoutes.map((tabRoute, index) => (
            <Tab
              key={tabRoute}
              data-testid={`tab-${tabRoute}`}
              sx={{
                ...getTabStyles({ isActive: activeTab === tabRoute, isFirst: index === 0 }), fontSize: '11.5px', textTransform: 'uppercase', letterSpacing: '0.4px'
              }}
            >
