...
export default function LastVoted({
  ...
  const lastVoteDate = date ? (
    <Flex>
      <Text sx={{ ml: [1, 0], fontSize: '12px' }}>LAST VOTED</Text> <DateWithHover timeago={!!isLongerThan14Days} date={date} />
    </Flex>
  ) : (
    'NO VOTE HISTORY'
  );
  ...
  return (
    ...
    >
      <Text variant="caps" color={expired ? '#D8E0E3' : 'onSecondary'} sx={{ mr: 2, ml: left ? 1 : 0, fontSize: '12px' }}>
        {lastVoteDate}
      </Text>
      <Flex
        sx={{
