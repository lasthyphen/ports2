...
export function PollVoteTypeIndicator({ poll }: { poll: Poll }): React.ReactElement {
  ...
  return (
    <Box>
          ...
          <TooltipComponent
            label={
              <Text sx={{ whiteSpace: 'normal' }}>
                ...
                winning vote option by finding the one with an absolute majority in HAL voting weight (as in
                &gt;50% of the total participating HAL, excluding abstains). In the first round of IRV, only
      ...
      {isPlurality && (
        <Flex sx={{ alignItems: 'center', fontSize: '10px' }}>
          <Text variant="caps">Plurality poll</Text>
          <TooltipComponent
            label={
              <Text sx={{ whiteSpace: 'normal', fontSize: '12px' }}>
                Plurality voting polls require single-choice ballots and determines the winning vote option by
                finding the one with the highest HAL voting weight in relative terms.
              </Text>
            }
          >
            <Flex sx={{ mb: '2px' }}>
          ...
          <TooltipComponent
            label={
              <Text sx={{ whiteSpace: 'normal' }}>
                ...
                winning vote option by finding the one with a relative majority in HAL voting weight. When
                used in situations where no winner is required, an absolute majority (ie. &gt;50% of the total
                participating HAL excluding abstains) victory condition may also be applied as opposed to a
                relative majority.
              </Text>
            }
          >
          ...
          <TooltipComponent
            label={
              <Text sx={{ whiteSpace: 'normal' }}>
                ...
                finding the one with an absolute majority in HAL voting weight, being &gt;50% of the total
                participating HAL (excluding abstains).
              </Text>
            }
          >
            <Flex sx={{ mb: '2px' }}>
