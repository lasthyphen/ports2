...
export const getStatusText = ({
  ...
  if (proposalAddress === ZERO_ADDRESS) {
    return `This proposal surpassed the 80,000 HAL threshold on ${formatDateWithTime(1607704862000)} – the new
  ...
  if (spellData.hasBeenScheduled || spellData.dateExecuted) {
    if (spellData.dateExecuted !== undefined && spellData.dateExecuted !== null) {
      return `Passed on ${formatDateWithTime(spellData.datePassed)}. Mandated on ${formatDateWithTime(
        spellData.dateExecuted
      )}.`;
    } else {
      return `Passed on ${formatDateWithTime(spellData.datePassed)}. Available for mandating on
      ${
  ...
  if (!!spellData.mkrSupport && !!mkrOnHat) {
    ...
    return `${formatValue(mkrNeeded)} additional HAL support needed to pass. Expires at ${formatDateWithTime(
      spellData.expiration
    )}.`;
  }

  // hasn't been scheduled, executed, hasn't expired, must be active and not passed yet
  return 'Proposal marked as Founders Mandate. Implementation is imminent. ';
};
