...
export default function DefaultVoteModalView({
  ...
  const votingMessage =
    currentSlate && currentSlate.includes(spellAddress) && currentSlate.length > 1
      ? 'Concentrate all my HAL on this proposal'
      : currentSlate && !currentSlate.includes(spellAddress) && isHat
      ? 'Add HAL to secure the protocol'
      : 'Submit Vote';
  ...
  return (
    <Flex sx={{ flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <Text sx={{ display: ['none', 'block'], marginTop: 3, color: 'onSecondary', fontSize: [3, 4] }}>
        You are voting for the following Governance Mandate:
      </Text>
      <Box
        sx={{
          mt: 2,
      ...
      >
        <Text as="p" sx={{ fontSize: 2, marginBottom: ['8px', '28px'] }}>
        ...
        <GridBox bpi={bpi}>
          ...
          {lockedMkr ? (
            <Text as="p" color="text" mt={[0, 2]} sx={{ fontSize: 3, fontWeight: 'medium' }}>
              {formatValue(lockedMkr, 'wad', 6)} HAL
            </Text>
          ) : (
            <Box sx={{ mt: [0, 2] }}>
              <SkeletonThemed />
      ...
      <Box sx={{ width: '100%' }}>
        ...
        {showHatCheckbox ? (
          ...
          >
            ...
            />
            Keep my HAL on old proposal to secure the Maker protocol
          </Label>
        ) : null}
      </Box>
    </Flex>
