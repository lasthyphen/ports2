...
async function getBalancerMkr(mkrAddress: string) {
  ...
  const balancerNum = json.data.pools
    ...
    .reduce((sum, token) => (token.symbol === 'HAL' ? parseFloat(token.balance) : 0) + sum, 0);
...
async function getCompoundMkr(network) {
  if (network !== SupportedNetworks.DIJETS) return BigNumber.from(0);

  const resp = await fetch('https://api.compound.finance/api/v2/ctoken');
  const json = await resp.json();

  const mkr = json.cToken?.find(token => token.underlying_symbol === 'WDJTX')?.cash?.value;
...
export default function MkrLiquiditySidebar({
      ...
      <Flex key={poolName} sx={{ flexDirection: 'column' }}>
        <Flex sx={{ justifyContent: 'space-between', flexDirection: 'row' }}>
          <Flex sx={{ alignItems: 'center' }}>
            <Text sx={{ fontSize: '14px', color: '#787a9b' }}>WDJTX in {poolName}</Text>
            {subpools && (
              <Box sx={{ ml: 1 }}>
                <Flex
                  sx={{
          ...
          <Text variant="h2" sx={{ fontSize: 3 }}>
            {poolLiquidity ? (
              `${formatValue(poolLiquidity)} WDJTX`
            ) : (
              <Box sx={{ width: 6 }}>
                <Skeleton />
              </Box>
          ...
          <Flex sx={{ flexDirection: 'column' }}>
            {subpools.map(subpool => {
              ...
              return (
                ...
                >
                  <Flex sx={{ alignItems: 'center' }}>
                    <Text sx={{ fontSize: '14px', color: 'textSecondary' }}>HAL in {subpoolName}</Text>
                  </Flex>
                  <Text variant="h2" sx={{ fontSize: '14px', color: 'textSecondary' }}>
                    {subpoolLiquidity ? (
                      `${formatValue(subpoolLiquidity)} WDJTX`
                    ) : (
                      <Box sx={{ width: 6 }}>
                        <Skeleton />
                      </Box>
  ...
  return (
    <Box sx={{ display: ['none', 'none'] }} className={className}>
      <Flex sx={{ alignItems: 'center', justifyContent: 'space-between', mb: 2, mt: 4 }}>
        <Heading as="h3" variant="microHeading">
          Wrapped DJTX Liquidity
        </Heading>
        <Text sx={{ fontSize: 4 }}>{totalLiquidity}</Text>
      </Flex>
      <Card variant="compact" sx={{background: 'linear-gradient(147.66deg, rgb(255, 255, 255) 0%, rgb(255, 255, 255)', padding: '32px', fontSize: '14px', color: '#8386ab'}}>
