...
export const useLock = (): LockResponse => {
  ...
  const lock = (mkrToDeposit: BigNumber, callbacks?: Record<string, () => void>) => {
    ...
    const transactionId = track(lockTxCreator, account, 'Depositing HAL', {
      pending: () => {
        if (typeof callbacks?.pending === 'function') callbacks.pending();
      },
      mined: txId => {
        transactionsApi.getState().setMessage(txId, 'HAL deposited');
        if (typeof callbacks?.mined === 'function') callbacks.mined();
      },
      error: txId => {
        transactionsApi.getState().setMessage(txId, 'HAL deposit failed');
